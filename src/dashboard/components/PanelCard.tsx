...
export default function PanelCard({
  ...
  return (
    <motion.section
      ...
      className={`dashboard-surface dashboard-interactive-card p-4 sm:p-5 lg:p-6 ${variantClasses[variant]} ${className}`}
    >
      ...
      {title || description || eyebrow || actions ? (
        <div className={`relative z-[1] flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between ${headerClassName}`}>
          <div className="min-w-0 max-w-3xl">
            ...
            {title ? (
              <h2 className={`mt-2 break-words text-[1.22rem] font-bold tracking-[-0.035em] text-white lg:text-[1.4rem] ${titleClassName}`}>
                {title}
              </h2>
            ) : null}
            {description ? (
              <p className={`mt-2 max-w-3xl text-[0.9rem] leading-6 text-slate-300 ${descriptionClassName}`}>
                {description}
              </p>
            ) : null}
          </div>
