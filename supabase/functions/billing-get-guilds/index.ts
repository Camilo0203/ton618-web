...
export function getSupabaseAuthHeaders(req: Request): string | null {
  return req.headers.get('Authorization');
}

export function createSupabaseAuthClient(authHeader: string) {
...
export function createBillingDiscordClient() {
  ...
  return new DiscordClient(discordClientId, discordClientSecret, discordRedirectUri);
}

export async function handleRequest(req: Request): Promise<Response> {
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: getCorsHeaders(getCorsOrigin(req)) });
  }
  ...
  try {
    ...
    const authHeader = getSupabaseAuthHeaders(req);
    if (!authHeader) {
      return errorResponse('Missing authorization header', 401);
    }

    const supabaseClient = createSupabaseAuthClient(authHeader);
    ...
    const discordClient = createBillingDiscordClient();
